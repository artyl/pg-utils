with s AS
(SELECT sum(total_exec_time) AS t, sum(blk_read_time+blk_write_time) as iot, sum(total_exec_time-blk_read_time-blk_write_time) as cput, sum(calls) AS s, sum(rows) as r FROM pg_stat_statements WHERE TRUE)
,
_pg_stat_statements as (
select dbid, regexp_replace(query, E'\\?(, ?\\?)+', '?') as query, sum(total_exec_time) as total_exec_time, sum(blk_read_time) as blk_read_time, sum(blk_write_time) as blk_write_time, sum(calls) as calls, sum(rows) as rows
...
(t/1000)*'1 second'::interval as total_exec_time,
...
(total_exec_time/1000)*'1 second'::interval as total_exec_time,
...
WHERE
(total_exec_time-blk_read_time-blk_write_time)/(SELECT cput FROM s)>=0.005
